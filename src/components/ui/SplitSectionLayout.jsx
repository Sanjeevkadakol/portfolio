...
export default function SplitSectionLayout({
    id,
    title,
    subtitle,
    subtitleIcon,
...
function ListItem({ icon, title, description, variants, delay, direction, link }) {
    const handleClick = () => {
        if (link && link !== "#") {
        ...
        >
            ...
            >
                ...
                <h3 className="text-xl font-medium text-white group-hover:text-[#88734C] transition-colors duration-300">
                    {title}
                    {link && <ArrowRight className="inline-block ml-2 w-4 h-4 opacity-0 group-hover:opacity-100 transition-opacity" />}
                </h3>
            </motion.div>
            <motion.p
...
function StatCounter({ icon, value, label, suffix, delay }) {
    ...
    useEffect(() => {
        if (isNumber && isInView && !hasAnimated) {
            springValue.set(value)
            setHasAnimated(true)
        } else if (isNumber && !isInView && hasAnimated) {
            springValue.set(0)
            setHasAnimated(false)
        }
    }, [isInView, value, springValue, hasAnimated, isNumber])
    ...
    return (
        ...
        >
            ...
            <div ref={countRef} className={`font-bold text-white flex items-center justify-center ${isNumber ? 'text-3xl' : 'text-lg text-center'}`}>
                {isNumber ? (
                    <>
            ...
            <motion.div className="w-10 h-0.5 bg-[#88734C] mt-3 group-hover:w-16 transition-all duration-300 mx-auto" />
        </motion.div>
    )
}
